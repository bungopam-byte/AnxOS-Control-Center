...
assert(sshSmoke.includes("sshXterm.onData") && sshSmoke.includes("writeSshInput(data);"), "SSH interactive smoke must cover xterm onData pass-through.");
assert(appSource.includes("sshXterm.onData((data) => {") && appSource.includes("writeSshInput(data);"), "SSH renderer must pass terminal input directly to the PTY.");
