...
async function main() {
  ...
  assert(htmlSource.includes('data-agent-control-section="credentials"') && htmlSource.includes('data-agent-control-action="generateToken"') && htmlSource.includes('data-agent-control-action="copyToken"'), "Agent Control Credentials must expose the manual Generate/Copy Token workflow.");
  ...
  assert(rendererSource.includes('action === "generateToken"') && rendererSource.includes("nodesApi.generateToken()"), "Agent Control token generation must use the existing trusted token IPC.");
  assert(rendererSource.includes('action === "copyToken"') && rendererSource.includes("activeAgentGeneratedToken"), "Agent Control token copy must use only the freshly generated token.");
  assert(rendererSource.includes('const agentPairingCodeInput = document.querySelector("textarea[data-agent-pairing-code]")'), "Remote Agent pairing must read from the pairing textarea, not the generated-code display.");
