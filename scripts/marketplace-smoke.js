...
function assertSingleDeviceModeExperience() {
  ...
  const accountSource = fs.readFileSync(path.join(__dirname, "..", "src", "services", "accountService.js"), "utf8");
  const accountIpcSource = fs.readFileSync(path.join(__dirname, "..", "src", "ipc", "accountIpc.js"), "utf8");
  const preloadSource = fs.readFileSync(preloadPath, "utf8");
  const accountPageSource = fs.readFileSync(path.join(__dirname, "..", "website", "account.html"), "utf8");
  const deviceLoginSource = fs.readFileSync(path.join(__dirname, "..", "website", "device-login.html"), "utf8");
  ...
  assert(preloadSource.includes("account:startDeviceLogin") && preloadSource.includes("account:checkDeviceLogin"), "Preload should expose account device-code APIs.");
  ...
  assert(accountSource.includes("ANXOS_ACCOUNT_API_URL") && accountSource.includes("device/start") && accountSource.includes("device/token"), "Account service should implement configurable device-code API integration.");
  assert(accountSource.includes("safeStorage") && accountSource.includes("account.json"), "Account service should persist account tokens securely.");
  ...
  assert(accountPageSource.includes("AnxOS Account") && deviceLoginSource.includes("Enter your device code"), "Website should include account and device-login pages.");
